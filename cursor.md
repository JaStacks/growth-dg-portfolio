...
**Stack:** Adapted for this repo: Next.js, Tailwind v4, Framer Motion (no GSAP).
...
- Use Framer Motion (`motion`, `useScroll`, `useTransform`, `AnimatePresence`) for all entrances, morphs, and scroll-linked effects.
- Easing: e.g. `[0.22, 1, 0.36, 1]` (similar to power3.out); use `transition={{ type: "tween", ease: [0.22, 1, 0.36, 1] }}` for entrances and `type: "spring"` where appropriate for morphs.
- Stagger: use `staggerChildren` / `delayChildren` with ~`0.08` for text, ~`0.15` for cards. Framer handles cleanup on unmount.
...
- **Morphing Logic:** Transparent with light text at hero top. Transitions to `bg-[background]/60 backdrop-blur-xl` with primary-colored text and a subtle `border` when scrolled past the hero. Use scroll state (e.g. `useScroll` from Framer or `IntersectionObserver`) to toggle.
...
- **Animation:** Framer Motion staggered fade-up (`initial={{ opacity: 0, y: 40 }}` → `animate={{ opacity: 1, y: 0 }}`) for all text parts and CTA, with stagger.
...
**Card 1 — "Diagnostic Shuffler":** 3 overlapping cards that cycle vertically using `array.unshift(array.pop())` logic every 3 seconds with a spring-bounce transition (`cubic-bezier(0.34, 1.56, 0.64, 1)` or Framer `type: "spring", bounce`). Labels derived from user's first value prop (generate 3 sub-labels).
...
- **Animation:** Framer Motion reveal (word-by-word or line-by-line fade-up via `staggerChildren`) triggered when in view (e.g. `whileInView` or `useInView`).

### E. PROTOCOL — "Sticky Stacking Archive"

3 full-screen cards that stack on scroll.

- **Stacking Interaction:** Use Framer Motion `useScroll` + `useTransform` to drive scale, blur, and opacity. As a new card scrolls into view, the card underneath scales to `0.9`, blurs to `20px`, and fades to `0.5`. Pin the active card in view while scrolling through the stack (scroll-driven state or layout).
...
- **Stack:** React 19, Next.js (this repo), Tailwind v4, Framer Motion, Lucide React.
- **Fonts:** Load via Next.js `next/font` (Google or local) in the page or `_app.js` based on the selected preset.
- **Images:** Use real Unsplash URLs. Select images matching the preset's `imageMood`. Use `next/image` for images. Never use placeholder URLs.
- **File structure:** Next.js page in `pages/` (e.g. `pages/landing.js` or a dedicated route) and components in `components/` (e.g. `components/landing/`). Use existing `styles/globals.css` (or project CSS) for Tailwind directives + noise overlay + custom utilities.
...
6. Add the page and components to this Next.js app (or create a new page/route), install any missing deps, write all files.
